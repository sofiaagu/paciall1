...

{
    ...
    public HazarZone[] hazarZones; // Arrastra la HazarZone del río
...

    {
...

        {
            ...
            if (hazarZones != null && spawnPointParte2 != null)
            {
                foreach (HazarZone zone in hazarZones)
                {
                    if (zone != null)
                    {
                        zone.currentRespawnPoint = spawnPointParte2;
                    }
                }
                Debug.Log("<color=yellow>¡Punto de Respawn actualizado en todas las HazarZones!</color>");
            }
